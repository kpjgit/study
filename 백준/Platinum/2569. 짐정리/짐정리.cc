...
int N;
std::pair<int, int> arr[1000]; //무게, 원래 idx
...
int main(void) {
	...
	int mn_arr = arr[0].first; //정렬 했으므로 가장 앞에 최소값이있다
	int ans = 0;
	for (int i = 0; i < N; i++) {
		if (vis[i]) continue;
		...
		int cycle_cost = 0;
		int cycle_length = 0;
		int mn_cycle = arr[i].first; //정렬을 하고 vis에 의해 앞 원소는 접근이 안 되니 i가 사이클 안에서의 최소값
		int cursor = i;
		while (!vis[cursor]) {
			vis[cursor] = true;
		...
		int cost_in_cycle = mn_cycle * (cycle_length - 2);
		int cost_out_cycle = mn_arr * (cycle_length + 1) + mn_cycle;
